...
<button type="button" class="btn btn-secondary" data-toggle="collapse" data-target="#Translate<%=this.Data[0].ID %>"><%=TextTranslate %></button>
  <div class="jumbotron mt-3 collapse" id="Translate<%=this.Data[0].ID %>">
...
<button type="button" class="btn btn-secondary" data-toggle="collapse" data-target="#Translate<%=this.Data[i].ID %>"><%=TextTranslate %></button>
  <div class="jumbotron mt-3 collapse" id="Translate<%=this.Data[i].ID %>">
...
<script>
    ...
    function LoadMore()
    {
        ...
        $.ajax({
            ...
            data: JSON.stringify({key: '<%= Session.SessionID%>', skip: currentbefore, take: <%= this.GetValueParam<int>("Top")%>, order: orderby, tag: key}),
            ...
            success: function (data) {
                if (data != "") {
                    ...
                    for (var i = 0; i < data.d.length; i++) {
                        ...
                        content +="<button type='button' class='btn btn-secondary' data-toggle='collapse' data-target='#Translate"+data.d[i].ID+"'><%=TextTranslate %></button>";
                        content +="<div class='jumbotron mt-3 collapse' id='Translate"+data.d[i].ID+"'>";
                        if(data.d[i].Title2)
                        {
