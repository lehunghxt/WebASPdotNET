...
<h1 title="<%=Title %>" class="main-heading" >
   <%=Title %>
    ...
    <asp:ListView ID="rpt" runat="server">
        <ItemTemplate>
            <div class="col-md-<%# ColumnCount == 1 || ColumnCount == 3 || ColumnCount == 4 || ColumnCount == 6 ? 12 / ColumnCount : 3%> col-sm-4 col-xs-6">
				<div class="product"  style="<%=WidthProduct > 0 ? "Width:" + WidthProduct + "px;": ""%><%=HeightProduct > 0 ? "Height:" + HeightProduct + "px;": ""%>">
                    ...
                    <div class="thumb img-shine" style="<%=WidthImage > 0 ? "Width:" + WidthImage + "px;": ""%><%=HeightImage > 0 ? "Height:" + HeightImage + "px;": ""%>">
						<a class="image img-scaledown" href="<%#HREF.LinkComponent("Product", SettingsManager.Constants.SendProduct + "/"+Eval("Id")+"/"+Eval("Title").ToString().ConvertToUnSign())%>" title="<%#Eval("Title")%>" style="<%=HeightImage > 0 ? "Height:" + HeightImage + "px;": ""%>">
                    ...
                    <div class="info">
                        <h2 class="title">
                            <a class="" href="<%#HREF.LinkComponent("Product", SettingsManager.Constants.SendProduct + "/"+Eval("Id")+"/"+Eval("Title").ToString().ConvertToUnSign())%>" title="<%#Eval("Title")%>">
                                <%#Eval("Title")%> <%if (this.GetValueParam<bool>("DisplayCode")){ %> - <span class="product-row-sku"><%#Eval("Code")%><%} %></span>
                            </a>
                        </h2>
                        <div class="price uk-flex uk-flex-middle uk-flex-space-between">
                            <%#Convert.ToDecimal(Eval("Price")) > 0 
                                    ? Convert.ToDecimal(Eval("Sale")) > 0 ? 
                                        "<span class='sale-price'>"+ String.Format("{0:0,0}", Eval("Sale")) + "<sup>đ</sup> "+ this.Unit +"</span><br><del>" + String.Format("{0:0,0}đ", Eval("Price")) + "<sup>đ</sup></del>" 
                                        : "<span class='sale-price'>"+ String.Format("{0:0,0}", Eval("Price")) +"<sup>đ</sup> "+ this.Unit +"</span>" 
                                    : "Liên hệ"%>
                        </div>
                        ...
                        <%if (HasOrder)
                            {
                                %>
                        <input data-toggle="modal" data-target="#buyModal" style='width:100%' class="addCarts btn btn-primary" onclick="SelectProduct('<%#Eval("Id")%>','<%#Eval("Title")%>','<%#Eval("ImagePath")%>','<%#Eval("SaleMin")%>','<%#Eval("Brief").ToString().Replace(",", " ").Replace("'", "\"")%>','<%#Convert.ToDecimal(Eval("Sale")) > 0 ? String.Format("{0:0,0}", Convert.ToDecimal(Eval("Sale"))) : Convert.ToDecimal(Eval("Price")) > 0 ? String.Format("{0:0,0}", Convert.ToDecimal(Eval("Price"))) : "Liên hệ"%>')" value="Mua ngay"></input>
                    <%} %>
                    </div>
				</div>
			</div> 
