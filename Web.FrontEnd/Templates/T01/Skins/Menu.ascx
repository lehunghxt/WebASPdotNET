...
<div id="departments-menu" class="dropdown departments-menu">
    ...
    <%if (item.Childs != null && item.Childs.Count > 0)
        ...
        <li itemscope="itemscope" itemtype="https://www.schema.org/SiteNavigationElement" id="departments-menu-menu-item-471" class="yamm-tfw menu-item menu-item-type-custom menu-item-object-custom menu-item-has-children menu-item-471 animate-dropdown dropdown-submenu">
            <a title="<%=item.CategoryName %>" data-toggle="dropdown" class="dropdown-toggle disabled" aria-haspopup="true" href="<a href="<%=CreateLink(item.Type, false, item.ID, item.CategoryName)%>" title="<%=item.CategoryName %>"><%=item.CategoryName %></a>"><%=item.CategoryName %>
                <span class="caret">
                </span>
            </a>
...
<%foreach (var dto in item.Childs)
                        {%>
                                            <div class="kc-elm kc-css-773924 kc_text_block">
                                                <h5><a href="<%=CreateLink(item.Type, false, dto.ID, dto.CategoryName)%>" title="<%=dto.CategoryName %>"><%=dto.CategoryName %></a></h5>
                                                <ul>
                                                    <%foreach (var con in dto.Childs)
                        {%>
                    <li><a href="<%=CreateLink(item.Type, false, con.ID, con.CategoryName)%>" title="<%=con.CategoryName %>"><%=con.CategoryName %></a></li>
                    <%} %>
                                                </ul>
                                            </div>
                                            <%} %>
