namespace TM.Website.Controllers
{
    ...
    public class LoginController : Controller
    {
        ...
        {
            // (new URM.Business.UserBLL()).Upgrade();
            ViewBag.Error = string.Empty;
            return View();
        }

        [HttpPost]
        public ActionResult Index(URMUserLoginModel model)
        {
            try
            {
                ApiHelper api = new ApiHelper();
                var user = api.Post<URMUserModel, URMUserLoginModel>(string.Format("http://{0}/api/Authenticate/Post", Request.Url.Authority), model);
        ...
        [HttpPost]
        public ActionResult ForgetPassword(URMForgetPasswordModel model)
        {
            try
            {
