namespace Web.Api.Controllers
{
    ...
    public class LinksController : ModuleController
    {
        protected CompanyConfigModel Web { get; set; }
        public LinksController(CompanyConfigModel web)
        {
            Web = web;
        }

        public ContentResult Links(string skinName, Dictionary<string, object> param, string title="")
        {
            var articleBLL = new ArticleBLL();
            var data = articleBLL.GetLinks(this.Web.ID, this.Web.Language).Where(e => e.PUBLISH).OrderBy(e => e.ORDERS).AsQueryable();
