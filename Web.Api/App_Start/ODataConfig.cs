...
namespace Web.Api
{
    ...
    {
        private const string namespaceModel = "Web.Api.Odata";
        /// <sURMmary>
        ...
        private static ODataConventionModelBuilder ResolveModelBuilder(Type entitySetControllerType)
        {
            ...
            if(entitySetControllerType.Namespace != namespaceModel)
                packageKey = entitySetControllerType.Namespace.Substring(namespaceModel.Length + 1);
        ...
        private static void RegisterNavigationEntitySet(Type entitySetControllerType, Type[] entitySetControllerTypes)
        {
            if (entitySetControllerType.BaseType != null && entitySetControllerType.BaseType.IsGenericType)
            {
                ...
                {
                    var type = navigationProperty.GetType();
                    if (navigationProperty.PropertyType.IsGenericType)
                        type = navigationProperty.PropertyType.GetGenericArguments()[0];
                    Type controllerOfNavigation =
                        entitySetControllerTypes.FirstOrDefault(
                            x =>
                            x.BaseType != null && x.BaseType.IsGenericType
                            && x.BaseType.GetGenericArguments()[0] == type);
                    if (controllerOfNavigation != null)
                    {
                        RegisterEntitySet(controllerOfNavigation, modelBuilder, type.Name);
                    }
                }
            }
        }
